...
// por defecto lee el token --color-red para no hardcodear la marca.
// Reutilizable: header (compact) y dashboard (grande).
...
const mix = (rgb: number[], t: number) => rgb.map((c) => Math.round(c + (255 - c) * t));

function accentFromToken(): string {
  if (typeof window === 'undefined') return '#FF2A2A';
  const v = getComputedStyle(document.documentElement).getPropertyValue('--color-red').trim();
  return v || '#FF2A2A';
}
...
export default function BrainCanvas({ accent, height = 140, active = false, compact = false }: Props) {
  ...
  useEffect(() => {
    ...
    const base = hexToRgb(accent || accentFromToken());
    const C = base.join(',');
